<xsl:stylesheet
  ...
  <xsl:template match="/">
        ...
        <xsl:variable name="ItemStyle" select="$Rows[1]/@Style" />
                 ...
                 <xsl:if test="$cbq_iseditmode = 'True' and string-length($cbq_errortext) != 0">
                  ...
                  <xsl:choose>
                      <xsl:when test="$IsEmpty">
                        ...
                        <xsl:when test="$ItemStyle = 'Blogs'">
                          <xsl:call-template name="OuterTemplate.Poll">
            ...
            <xsl:if test="$FeedEnabled = 'True' and $PageId != ''">
                <div class="cqfeed">
                    ...
                    <a href="{cmswrt:RegisterFeedUrl( $FeedUrl1, 'application/rss+xml')}"><img src="/_layouts/15/images/rss.gif?rev=33" border="0" alt="{cmswrt:GetPublishingResource('CbqRssAlt')}"/></a>
                </div>
            </xsl:if>
    </xsl:template>
    ...
    <xsl:template name="OuterTemplate.Blogs">
      <xsl:param name="Rows" />
      <xsl:param name="FirstRow" />
      <xsl:param name="LastRow" /> 
      <xsl:variable name="BeginColumn1" select="string('&lt;section id=&quot;blogs-rollup&quot; class=&quot;blogs-rollup content-block&quot;&gt;&lt;h1&gt;BLOGS&lt;/h1&gt;')" />
      <xsl:variable name="BeginColumn" select="$BeginColumn1" />
      <xsl:variable name="EndColumn" select="string('&lt;/section&gt;')" />
      <xsl:for-each select="$Rows">
            <xsl:variable name="CurPosition" select="position()" />
            <xsl:if test="($CurPosition &gt;= $FirstRow and $CurPosition &lt;= $LastRow)">
                <xsl:variable name="StartNewGroup" select="@__begingroup = 'True'" />
          ...
          <span class="ms-imnSpan">
                <a href="#" onclick="IMNImageOnClick(event);return false;" class="ms-imnlink">
                    <span class="ms-spimn-presenceWrapper ms-imnImg ms-spimn-imgSize-10x10">
                        <img src="/_layouts/15/images/spimn.png?rev=33" class="ms-spimn-img ms-spimn-presence-disconnected-10x10x32" onload="IMNRC('{@SipAddress}')" ShowOfflinePawn="1" alt="" id="{concat('MWP_pawn_',$ClientId,'_',@ID,'type=sip')}"/>
                    </span>
                </a>
            </span>
        </xsl:if>
